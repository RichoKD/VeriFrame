...
class Job(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    chain_job_id = Column(Integer, unique=True, nullable=False, index=True)  # On-chain job ID
    
    # Job details
    creator_address = Column(String(66), nullable=False, index=True)
    asset_cid_part1 = Column(String(255), nullable=False)
    asset_cid_part2 = Column(String(255), nullable=True)
    full_asset_cid = Column(String(255), nullable=True)  # Combined CID
    
    # Reward and timing
    reward_amount = Column(BigInteger, nullable=False)  # In wei
    ...
    full_result_cid = Column(String(255), nullable=True)  # Combined CID
    quality_score = Column(Integer, nullable=True)  # 0-100
    
    # Status
    status = Column(String(50), default="open")  # open, assigned, completed, failed, cancelled
